...
def list_clients(db: Session = Depends(get_db)):
    ...
    res = []
    for c in clients:
        # Filtrar o sanear registros vacíos o corruptos
...
<head>
    <meta charset="UTF-8">
    <title>JiraTeamsSync - Gestión Multicliente</title>
    <style>
        ...
        .card { background: #1e293b; border: 1px solid #334155; border-radius: 8px; padding: 20px; margin-bottom: 25px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.2); }
        .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }
        label { display: block; font-size: 12px; color: #94a3b8; font-weight: bold; margin-top: 10px; }
        input, select, textarea { width: 100%; padding: 8px; background: #0f172a; border: 1px solid #334155; color: #fff; border-radius: 4px; box-sizing: border-box; }
        ...
        .table-scroll { max-height: 400px; overflow-y: auto; border: 1px solid #334155; margin-top: 10px; }
...
<div class="container">
    ...
    <!-- Frecuencia -->
    <div class="card">
        <h2>⏱️ Frecuencia de Sincronización</h2>
        <div class="grid">
            <div><label>Intervalo periódico (minutos):</label><input type="number" id="app_interval" value="15"></div>
    ...
    <div class="card">
        ...
        <div class="grid">
            <div>
                <label>Nombre:</label><input type="text" id="c_name" placeholder="FedPat" required>
                <label>Dominio Jira:</label><input type="text" id="c_domain" placeholder="fpatronal.atlassian.net" required>
                <label>Email:</label><input type="email" id="c_email" placeholder="usuario@empresa.com" required>
            </div>
            <div>
                <label>API Token:</label><input type="password" id="c_token" required>
                <label>Horario Operativo (HH:MM):</label>
                <div style="display:flex; gap:10px;">
                    <input type="text" id="c_start" value="07:00">
                    <input type="text" id="c_end" value="20:00">
                </div>
                <label>Host VPN (opcional):</label><input type="text" id="c_vpn" placeholder="vpn.empresa.com">
            </div>
        </div>
    ...
    <div class="card" id="boardsSection" style="display:none;">
        ...
        <h4 style="margin-top:20px; color:#38bdf8;">🗂️ Mapeo y Uso de Campos de Jira</h4>
...
<script>
    ...
    function loadClients() {
        fetch('/api/clients')
            ...
            .then(data => {
                ...
                let h = '<table><tr><th>Nombre</th><th>Dominio</th><th>Horario</th><th>Tableros</th><th>Acciones</th></tr>';
                data.forEach(c => {
            ...
            .catch(err => {
                ...
                document.getElementById('clientsList').innerHTML = '<p style="color:#f87171;">Error al cargar la lista de clientes.</p>';
            });
    }
    ...
    function createClient() {
        ...
        if(!name || !domain || !email || !token) {
            alert('Por favor, completa todos los campos requeridos (Nombre, Dominio, Email y Token).');
            return;
        }
